...
const signUp = async (req, res) => {
    ...
    console.log(process.env.TEACHER_SECRET+"    process.env.TEACHER_SECRET");
    console.log(teacherCode+"      teacherCode");
    
    const role = (teacherCode === process.env.TEACHER_SECRET) ? 'lacturer' : 'student';
     console.log(role);
    ...
    if (newUser) {
        return res.status(201).json({ message: `New user ${newUser.userName} created as ${role}` });
    } else {
