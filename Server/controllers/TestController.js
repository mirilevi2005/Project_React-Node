  ...
  const mailOptions = {
    ...
    html: `
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
        <h2 style="color: #1976d2;">שלום ${fullName},</h2>
        <p>סיימת את המבחן "<strong>${testTitle}</strong>".</p>
        <p style="font-size: 18px; font-weight: bold; color: #388e3c;">הציון שלך הוא: ${score} מתוך 100</p>
        <p>כל הכבוד על ההשקעה! מאחלים לך הצלחה בהמשך הדרך.</p>
        <p style="font-size: 14px; color: #999;">בברכה,<br>צוות EduThec</p>
      </div>
    `,
  };
...
exports.getUpcomingTestsForStudent = async (req, res) => {
  try {
    ...
    console.error("Error getting upcoming tests for student:", error);
