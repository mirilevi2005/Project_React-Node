...
import "../css/VideoOfMaterial.css";
import { useAddMaterialMutation } from "../redux/slice/api/materialsApi";
import { useCreateTestMutation, useGetTestsByCourseQuery, useUpdateTestMutation } from "../redux/slice/api/testApi";
import { addVideo } from "../redux/slice/videoSlice";
import VideoList from "./VideoList";
...
const VideoOfMaterial: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      if (isEditMode && editingTestId) {
        ...
        // await updateTest({ testId: editingTestId, updatedTest: testData }).unwrap();
        await updateTest({ id: editingTestId, updatedData: testData }).unwrap();
  ...
  return (
    <div className="upload-container">
      ...
      {/* Test Dialog */}
      <Dialog open={openTestDialog} onClose={handleCloseTestDialog} maxWidth="lg" fullWidth>
        <DialogTitle>{isEditMode ? "עריכת מבחן" : "יצירת מבחן חדש"}</DialogTitle>
        <DialogContent>
          <form onSubmit={handleSubmit(onSubmit)}>
            <Stack spacing={2}>
              <TextField
              ...
              <Button type="submit" variant="contained" color="primary">
                {isEditMode ? "שמור שינויים" : "שמור מבחן"}
              </Button>
            </Stack>
          </form>
        </DialogContent>
              ...
              {testList.map((test) => (
                <Card key={test._id} variant="outlined">
                  <CardContent>
                    <Typography variant="h6">{test.title}</Typography>
                    <Typography>תאריך אחרון: {new Date(test.lastDate).toLocaleString()}</Typography>
                    <Button variant="outlined" onClick={() => handleEditTest(test)} sx={{ mt: 1 }}>
                      ערוך
                    </Button>
                  </CardContent>
                </Card>
