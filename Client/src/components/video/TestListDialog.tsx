...
//   open: boolean;
//   onClose: () => void;
//   onEditTest: (test: IFormInput) => void;
//   tests: IFormInput[];  // או סוג מתאים למבחנים שלך
// }


// const TestListDialog = ({ open, onClose, onEditTest, tests}: TestListDialogProps) => {
//   const [selectedTest, setSelectedTest] = useState<IFormInput | null>(null);
//   const urlParts = window.location.pathname.split("/");
//   const courseName = urlParts[urlParts.length - 1];

//   const [deleteTest] = useDeleteTestMutation();
//   const { data, refetch, isLoading } = useGetTestsByCourseForTeacherQuery(courseName);
//   const testList = data?.tests ?? [];

//   const handleDeleteTest = async (testId: string) => {
//     if (!window.confirm("האם את בטוחה שברצונך למחוק את המבחן?")) return;
//     try {
//       await deleteTest(testId).unwrap();
//       await refetch();
//       setSelectedTest(null);
...
//               <Box key={idx} mb={2} p={1} border={1} borderRadius={2}>
//                 <Typography>שאלה {idx + 1}: {q.text}</Typography>
//                 {q.answers.map((opt, i) => (
//                   <Typography
//                     key={i}
//                     sx={{ ml: 2 }}
//                     color={q.correct === i ? "green" : "inherit"}
//                   >
//                     תשובה {i + 1}: {opt}
//                   </Typography>
//                 ))}
...
//                       תאריך אחרון: {new Date(test.LastDate).toLocaleString()}
//                     </Typography>
//                     <Button onClick={(e) => { e.stopPropagation(); onEditTest(test); }} sx={{ mt: 1 }} variant="outlined" color="primary">
//                       ערוך
//                     </Button>
//                   </CardContent>
//                 </Card>
...
//               חזרה לרשימה
//             </Button>
//             <Button onClick={() => handleDeleteTest(selectedTest._id)} color="error">
//               מחק מבחן
//             </Button>
//           </>
//         ) : null}
