  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        setUploadMessage('No response received from server');
      }
    } catch (error: any) {
      console.error('Error uploading video:', error);
      setUploadSuccess(false);
      setUploadMessage(error.message ? `Error: ${error.message}` : 'An error occurred while uploading the video');
    } finally {
  ...
  return (
    <Box sx={{ mt: 2 }}>
      <Paper elevation={0} sx={{ p: 3, backgroundColor: 'rgba(0, 0, 0, 0.02)', borderRadius: 2, mb: 3 }}>
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Typography variant="body1">
            Upload a new course video by clicking the button
          </Typography>
        </Stack>
      </Paper>
      ...
      <Dialog 
        ...
        maxWidth="sm"
      >
        <DialogTitle>Video Upload Settings</DialogTitle>
        <DialogContent dividers>
          ...
          <Typography variant="body2" gutterBottom sx={{ mt: 2 }}>
            Select the date and time until which the video will be available for students:
          </Typography>
