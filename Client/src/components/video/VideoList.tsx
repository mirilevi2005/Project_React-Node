...
const VideoList = ({ courseName }: Props) => {
  ...
  const handleDelete = async (videoId: string, videoPublicId: string) => {
    try {
      // מחיקת הסרטון מ-Cloudinary באמצעות public_id
      await deleteMaterial({ videoId, publicId: videoPublicId }).unwrap();
      dispatch(deleteVideo(videoId));
      refetch();
    } catch (error) {
      console.error("Error deleting video:", error);
    ...
    <div className="video-list">
      {videos.map((video) => (
        <div key={video._id} className="video-item" style={{ marginBottom: "24px" }}>
          ...
          <video width="600" controls>
            <source
              src={`${video.videoUrl}`}  // שימוש ב-Cloudinary
              type="video/mp4"
            />
            הדפדפן שלך לא תומך בווידאו.
          </video>   
          <p>{video.videoUrl}</p>         
          <Stack direction="row" spacing={1} sx={{ alignItems: "center", mt: 1 }}>
            <IconButton onClick={() => handleDelete(video._id, video.cloudinaryPublicId)} aria-label="delete" size="large">
              <DeleteIcon />
            </IconButton>
          </Stack>
        </div>
